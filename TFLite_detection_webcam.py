...
#TODO create a shutdown script 
#initiliaze to zero. 
GPIO.output(leftM1, 0)
GPIO.output(leftM2, 0)
GPIO.output(rightM1, 0)
GPIO.output(rightM2, 0)
GPIO.output(lidM1, 0)
GPIO.output(lidM2, 0)
...
def moveForward():
    GPIO.output(leftM1, 1)
    GPIO.output(leftM2, 0)   
    GPIO.output(rightM1, 0)
    GPIO.output(rightM2, 1)
...
def openLid():
    ...
    moveMotor = false
    if buttonState(limOpen) == False:    
        #not pressed
        moveMotor = True
        time.sleep(1)
        print("Opening the lid, waiting for ")
    
    while moveMotor == True:
        GPIO.output(lidM1, 0)  
        GPIO.output(lidM2, 1) 
        if buttonState(limOpen) == True:
...
def closeLid():
    #move until the other switch is pressed
    moveMotor = false
    if buttonState(limClose) == False:    
        #not pressed
        moveMotor = True
        timer.sleep(1)
        print("Closing the lid, waiting for button to be pressed")
    
    while moveMotor == True:
        GPIO.output(lidM1, 1)  
        GPIO.output(lidM2, 0) 
        if buttonState(limClose) == True:
            #reached open lid switch means it's opened
            GPIO.output(lidM1, 0)  
            GPIO.output(lidM2, 0)
            moveForward = False
